...
Esta arquitectura se basa en que se muestra en [Ejecución de una máquina virtual Linux en Azure][single vm]. Las recomendaciones que ahí se encuentran se aplican también a esta arquitectura.
...
Coloque las máquinas virtuales en la misma subred. No exponga las máquinas virtuales directamente a Internet; en su lugar, asigne una dirección IP privada a cada máquina virtual. Los clientes se conectan con la dirección IP pública del equilibrador de carga.
...
Hay disponible una implementación de esta arquitectura en [GitHub][github-folder]. Incluye una red virtual, un grupo de seguridad de red (NSG) y tres máquinas virtuales en un conjunto de escalado detrás de un equilibrador de carga, que se describe a continuación:
